...
pub fn get_zheyuye_edu_str(edus: &Option<Vec<CvEduResp>>) -> String {
    match edus {
        Some(edu) => {
...
pub fn gen_zheyuye_work_items(content: String) -> String {
    if content.is_empty() {
        return content;
    }
...
pub fn get_zheyuye_work_str(works: &Option<Vec<CvWorkResp>>) -> String {
    match works {
        Some(edu) => {
            let mut s = String::new();
            for i in edu {
                ...
                let work_item_content =
                    gen_zheyuye_work_items(i.duty.as_deref().unwrap_or_default().to_string());
                s += &format!(
                    "{}{}{}{}{}{}{}{}{}{}{}{}{}",
                    "\\datedline{\\textbf{",
                    i.company,
...
pub fn get_zheyuye_skill_str(works: &Option<Vec<CvSkillResp>>) -> String {
    match works {
        Some(edu) => {
            if edu.is_empty() {
                return "".to_owned();
...
pub fn get_zheyuye_lang_str(works: &Option<Vec<CvLangResp>>) -> String {
    match works {
        Some(edu) => {
            let mut s = String::from("\\begin{itemize}[parsep=0.5ex]\n");
            for i in edu {
...
pub fn get_zheyuye_project_str(works: &Option<Vec<CvProjectResp>>) -> String {
    match works {
        Some(edu) => {
            let mut s = String::new();
            for i in edu {
